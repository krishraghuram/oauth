...
class LoginSuccessView(View):
	def get(self, request, *args, **kwargs):
		...
		# 	text = "Hello " + user + ". Login Sucessful. Redirecting."
		# else:
		# 	redirect_to = reverse('login')
		# 	text = "You are not logged in. Redirecting to login page."
		# context = {'next':redirect_to, 'text':text}
		# return render(request, 'testoauth/login_success.html', context=context)
		time.sleep(1)
		if request.user.is_authenticated:
			messages.info(request, "You are logged in as "+request.user.get_short_name())
			return render(request, 'testoauth/login_success.html')
		else:
			redirect_to = reverse('login')
			text = "You are not logged in. Redirecting to login page."
			context = {'next':redirect_to, 'text':text}	
